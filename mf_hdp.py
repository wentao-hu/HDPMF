...
def evaluate(model, test_ratings,user_privacy_vector,item_privacy_vector):
    ...
    for i in range(len(test_ratings)):
        ...
        if user in user_privacy_vector.keys() and item in item_privacy_vector.keys():
            num_test_examples+=1
            # #without rescaling scheme, only for ablation
            # prediction=np.dot(model.user_embedding[user], model.item_embedding[item])
            
            #with rescaling scheme
            prediction=np.dot(model.user_embedding[user]/user_privacy_vector[user], model.item_embedding[item]/item_privacy_vector[item])
            if prediction>=5:  
                prediction=5
            if prediction<=1:  
                prediction=1
...
def main():
    ...
    parser.add_argument('--max_budget',type=float,default=1.0,help='maximum privacy budget for all the ratings')
    parser.add_argument('--user_ratio',type=str,default="0.54 0.37 0.09",help='ratio for 3 types of users')
    parser.add_argument('--item_ratio',type=str,default="0.3333 0.3333 0.3334",help='ratio for 3 types of items')
    parser.add_argument('--user_privacy',type=str,default='0.1 0.5 1',help='privacy weight list for different type of uses')
    parser.add_argument('--item_privacy',type=str,default="0.1 0.5 1",help='privacy weight list for different type of items')
    parser.add_argument('--filename',type=str,default="./Results/hdp.csv",help='filename to store the training and testing result')
    parser.add_argument('--logfile',type=str,default="./log/hdp.log",help='path to store the log file')
    # hyperparameter
    parser.add_argument('--lr',type=float,default=0.01,help='initial learning rate') 
    parser.add_argument('--embedding_dim',type=int,default=10,help='Embedding dimensions')
    parser.add_argument('--regularization',type=float,default=0.01,help='L2 regularization for user and item embeddings.')             
    parser.add_argument('--stddev',type=float,default=0.1,help='Standard deviation for initialization.')    
    ...
    try:
        if args.mode=="cv":
            ...
            train_ratings=load_rating_file_as_list(f"{args.data}/u{i}.base")
            test_ratings=load_rating_file_as_list(f"{args.data}/u{i}.test")
        else: #args.mode=="test"
        ...
        for epoch in range(total_epochs):
            ...
            #for large dataset, output test_error only 3 times, speedup nearly 10 times
            if epoch==99:
